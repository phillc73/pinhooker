#' getTatts - Download historic sale data from Tattersalls and Tattersalls
#' Ireland.
#'
#' \code{getTatts} downloads historic sale data from the Tattersalls and
...
#' \code{sqlite}. Only a csv is generated by default. Valid URLs for Tattersalls
#' and Tattersalls Ireland sales have been tested as far back as 2009. The
#' \link{scripts} folder contains a complete set of URLs and function calls to
#' generate data for all sales back to 2009.
#'
#' @param url A string containing the universal resource locator for an xls file
#'   of historic bloodstock sale data. Required, no default set.
...
#'   the \link{data} directory.
#'
#' @return If all parameters are valid, xls data will be downloaded, normalised
#'   and output, as the specified file fomats, in the \link{data} directory.
#'
#' @example getTatts(url =
#' "http://db.tattersalls.com/4DCGI/Entry/List?30%096%09=%20DEM%2015%09by8",
#' catalogue = "http://docs.tattersalls.com/cat/december/2015decmares.pdf", csv
#' = FALSE, rds = TRUE, sqlite = FALSE, auctioneer = "Tattersalls", country =
#' "UK", currency = "GNS", date = "2015-11-30", sale = "December Mares Sale",
#' filename = "tattsSaleData")
#'

getTatts <-
  function(url, catalogue = "", auctioneer, country, currency, date, csv = TRUE, rds = TRUE, sqlite = TRUE, sale = "", filename = "bloodstockSalesData") {
    # Scrape URL
    ...
    lot <-
      as.list(
        tattsData %>% html_nodes(".row2+ .odd .lhs , .row2+ .even .lhs , br+ .listTable .odd:nth-child(3) .lhs") %>%
          html_text()
      )
    lot <- unlist(lot)
    ...
    nameSexSireDam <-
      as.list(
        tattsData %>% html_nodes(
          ".row2+ .odd .lhs+ td , .row2+ .even .lhs+ td , br+ .listTable .odd:nth-child(3) .lhs+ td"
        ) %>%
          html_text()
      )
    ...
    name <- sub("(.*?)).*", "\\1)", nameSexSireDam$nameSexSireDam) %>%
    ...
    consignorPurchaser <-
      as.list(tattsData %>% html_nodes("br+ .listTable .row2 .lhs+ td") %>%
                html_text())
